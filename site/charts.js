(function () {
  ...
  function render(cv) {
    ...
    return fetch(BASE + '/data/' + slug + '.json').then(function (r) {
      ...
      g.addColorStop(0, 'rgba(111,174,133,.25)');
      g.addColorStop(1, 'rgba(111,174,133,0)');
      new Chart(cv, {
        type: 'line',
        data: { labels: labels, datasets: [{ data: values, borderColor: '#6fae85', borderWidth: 2, pointRadius: 0, fill: true, backgroundColor: g, tension: .25 }] },
        options: {
          ...
          plugins: {
            ...
            tooltip: {
              backgroundColor: '#26272b', titleColor: '#fff', bodyColor: '#c4c4c4', displayColors: false,
              callbacks: {
          ...
          scales: {
            x: { ticks: { color: '#8a8a8a', maxTicksLimit: 6, maxRotation: 0, callback: function (v) { return labels[v].slice(0, 4); } }, grid: { display: false } },
            y: { ticks: { color: '#8a8a8a', maxTicksLimit: 5, callback: function (v) { return fmt(v); } }, grid: { color: 'rgba(255,255,255,.06)' }, border: { display: false } }
          }
        }
      });
    });
  ...
  function init() {
    document.querySelectorAll('canvas[data-series]').forEach(function (cv) {
      render(cv).catch(function (e) {
        console.error(e);
        cv.parentNode.innerHTML = '<p style="color:#777;font-size:12px">Chart temporarily unavailable.</p>';
      });
    });
  }
